...
znewdat = expand.grid(dev.300 = seq(0, 1, by = 0.1),
  ...
  mutate(dev.60 = dev.300)



zpred_wt = predict(wt_crossings_mods_puma$dev60.bridge, znewdat, se.fit = TRUE, type = "response", re.form = NA) %>% 
  data.frame() %>% 
  bind_cols(znewdat) %>% 
...
ggsave(here("figures/wt_mod_plot.png"), width = 5)
