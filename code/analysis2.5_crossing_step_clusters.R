...
crossing_steps <- distinct(road_crossing_steps, step.id)
...
system.time(
  neighbor_steps_all <- map2_df(distinct(road_crossing_steps, step.id)$step.id, 2, extract.with.context), gcFirst = TRUE
)
