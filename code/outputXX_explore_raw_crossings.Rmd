...
After basic data cleaning, the first major step in this analysis is to identify which mountain lion "steps" crossed roads. A step is simply the movement between 2 sequential GPS locations. Below I consider all steps that were between 45 minutes and 12 hours.
...
Eliminating all these assumed lower risk roads from the operations described below greatly improves computing time for detecting which steps involved road crossings. In the map below the higher risk roads are colored by road class, and the lower risk roads are gray.  
...
ggplot(out_roads)  +
  ...
  geom_segment(data = filter(zout_crossings, (between(easting, bb.xmin, bb.xmax) & between(easting.end, bb.xmin, bb.xmax)) & 
...
The dataset contains fewer steps during daylight. This is NOT a reflection of mountain lion behavior, and is only due to collar performance and data filtering. This needs to be investigated further. However, this plot does show a strong tendency for road crossing steps to be during night.   
...
And among longer steps the average lengths of crossing and non-crossing steps are much more similar and with stronger overlapping standard deviations than when comparing all steps. This suggests that mountain lions don't necessarily encounter and cross roads every time they make longer steps, but when they do cross roads, they do so withlonger steps.
...
Viewing step length as a function of hour of day reveals that steps were shorter during the early daylight hours and longest in the first few hours of darkness for both crossing and non-crossing steps. 
